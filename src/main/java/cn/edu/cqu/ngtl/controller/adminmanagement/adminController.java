...
//        logger.error("进入管理台出错");
    ...
    public ModelAndView saveTimeCategoryPage(@ModelAttribute("KualiForm") UifFormBase form) {
        ...
        logger.info("保存了"+timeSettingType.getTypeName()+"时间类别");
    ...
    public ModelAndView deleteTimeCategoryPage(@ModelAttribute("KualiForm") UifFormBase form) {
        ...
        logger.info("删除了"+timeSettingType.getTypeName()+"时间类别");
        return this.getTimeCategoryPage(infoForm);
    }
...
//        return this.getModelAndView(infoForm, "pageWorkFlowCategory");
    ...
    public ModelAndView deleteTimeSetting(@ModelAttribute("KualiForm") UifFormBase form) {
        ...
        logger.info("删除类型为"+selectedTimeSettings.getTimeSettingType().getTypeName()+"、起始时间为"+selectedTimeSettings.getStartTime()+"、结束时间为"+selectedTimeSettings.getEndTime()+"、修改时间为"+selectedTimeSettings.getEditTime()+"的一个时间段。");
    ...
    public ModelAndView deletePermission(@ModelAttribute("KualiForm") UifFormBase form, HttpServletRequest request,
        ...
        int index = params.getSelectedLineIndex();
        KRIM_PERM_T selectedKrim_perm_t = infoForm.getRMPkrimPermTs().get(index);
        if (selectedKrim_perm_t.getActive().equals("Y"))
         active="可用";
        else
            active="失效";
    ...
    public ModelAndView saveURMPuser(@ModelAttribute("KualiForm") UifFormBase form, HttpServletRequest request,
        ...
        String message="保存了赋予给"+infoForm.getURMutInstructor().getName()+"用户的角色名称为";
    ...
    public ModelAndView getCourseManagerPage(@ModelAttribute("KualiForm") UifFormBase form) {
        ...
        infoForm.setCourseManagerViewObjects(adminConverter.getCourseManagerToTableViewObject(adminService.getCourseManagerByUid(uId)));
...
//        infoForm.setHistogram(histojson);
     ...
     * @return
     */
    @RequestMapping(params = {"pageId=pageFundsManagement","methodToCall=exportDetailFundingExcel"})
    public ModelAndView  exportDetailFundingExcel(@ModelAttribute("KualiForm") UifFormBase form){
        AdminInfoForm infoForm=(AdminInfoForm) form;
        this.baseStart(infoForm);

        if(infoForm.getDetailFunding()==null || infoForm.getDetailFunding().size()==1){//size=1是因为会设置至少一个空object让表格不会消失
            if(infoForm.getDetailFunding().get(0).getStuId()==null){
                infoForm.setErrMsg("列表为空！");
                return this.showDialog("refreshPageViewDialog",true,infoForm);
            }
        }

        List<DetailFundingViewObject> detailFundingViewObjectList=infoForm.getDetailFunding();
        SimpleDateFormat curTime=new SimpleDateFormat("yyyy-MM-dd");
        String fileName="经费明细列表"+"-"+getUserSession().getLoggedInUserPrincipalId()+"-"+curTime.format(new Date())+".xls";

        try{
            String filePath=excelService.printDetailFundingExcel(detailFundingViewObjectList,"exportfolder",fileName,"2003");
            String baseUrl=CoreApiServiceLocator.getKualiConfigurationService().getPropertyValueAsString(
     ...
     * @return
     */
    @RequestMapping(params = {"pageId=pageFundsManagement","methodToCall=exportPreviousSessionFundingsExcel"})
    public ModelAndView exportPreviousSessionFundingsExcel(@ModelAttribute("KualiForm") UifFormBase form){
        AdminInfoForm infoForm=(AdminInfoForm) form;
        this.baseStart(infoForm);

        if(infoForm.getPreviousSessionFundings()==null || infoForm.getPreviousSessionFundings().size()==1){//size=1是因为会设置至少一个空object让表格不会消失
            if(infoForm.getPreviousSessionFundings().get(0)==null){
                infoForm.setErrMsg("列表为空！");
                return this.showDialog("refreshPageViewDialog",true,infoForm);
            }
        }

        List<SessionFundingViewObject> sessionFundingViewObjectList=infoForm.getPreviousSessionFundings();
        SimpleDateFormat curTime=new SimpleDateFormat("yyyy-MM-dd");
        String fileName="学校历史经费列表"+"-"+getUserSession().getLoggedInUserPrincipalId()+"-"+curTime.format(new Date())+".xls";

        try{
            String filePath=excelService.printPreviousSessionFundingsExcel(sessionFundingViewObjectList,"exportfolder",fileName,"2003");
            String baseUrl=CoreApiServiceLocator.getKualiConfigurationService().getPropertyValueAsString(
                    KRADConstants.ConfigParameters.APPLICATION_URL);
            return this.performRedirect(infoForm,baseUrl+"/"+filePath);
        }catch(IOException e){
    ...
    public ModelAndView exportDepartmentCurrFundingsExcel(@ModelAttribute("KualiForm") UifFormBase form){
        ...
        if(infoForm.getDepartmentCurrFundings()==null || infoForm.getDepartmentCurrFundings().size()==1){//size=1是因为会设置至少一个空object让表格不会消失
            if(infoForm.getDepartmentCurrFundings().get(0)==null){
                infoForm.setErrMsg("列表为空！");
                return this.showDialog("refreshPageViewDialog",true,infoForm);
            }
        }

        List<DepartmentFundingViewObject> departmentFundingViewObjectList=infoForm.getDepartmentCurrFundings();
        SimpleDateFormat curTime=new SimpleDateFormat("yyyy-MM-dd");
        String fileName="学院经费列表"+"-"+getUserSession().getLoggedInUserPrincipalId()+"-"+curTime.format(new Date())+".xls";

        try{
            String filePath=excelService.printDepartmentCurrFundingsExcel(departmentFundingViewObjectList,"exportfolder",fileName,"2003");
            String baseUrl=CoreApiServiceLocator.getKualiConfigurationService().getPropertyValueAsString(
                    KRADConstants.ConfigParameters.APPLICATION_URL);
            return this.performRedirect(infoForm,baseUrl+"/"+filePath);
        }catch(IOException e){
     ...
     * @return
     */
    @RequestMapping(params = {"pageId=pageFundsManagement","methodToCall=exportDepartmentPreFundingsExcel"})
    public ModelAndView exportDepartmentPreFundingsExcel(@ModelAttribute("KualiForm") UifFormBase form){
        AdminInfoForm infoForm=(AdminInfoForm) form;
        this.baseStart(infoForm);

        if(infoForm.getDepartmentPreFundings()==null || infoForm.getDepartmentPreFundings().size()==1){//size=1是因为会设置至少一个空object让表格不会消失
            if(infoForm.getDepartmentPreFundings().get(0)==null){
                infoForm.setErrMsg("列表为空！");
                return this.showDialog("refreshPageViewDialog",true,infoForm);
            }
        }
     ...
     *
     */
    @RequestMapping(params = {"pageId=pageFundsManagement","methodToCall=exportClassFundingsPDF"})
    public ModelAndView exportClassFundingsPDF(@ModelAttribute("KualiForm") UifFormBase form){
        AdminInfoForm infoForm = (AdminInfoForm) form;
        super.baseStart(infoForm);

        if (infoForm.getClassFundings() == null || infoForm.getClassFundings().size() == 1) { //size=1是因为会设置至少一个空object让表格不会消失
            if(infoForm.getClassFundings().get(0) == null) {
                infoForm.setErrMsg("列表为空！");
                return this.showDialog("refreshPageViewDialog", true, infoForm);
            }
        }
        List<ClassFundingViewObject> classFundingViewObjectList=infoForm.getClassFundings();

        String filePath=ipdfService.prepareClassFundingsToPDF(classFundingViewObjectList);
        if (filePath.equals("exception")){
            infoForm.setErrMsg("系统导出PDF文件错误！");
            return this.showDialog("refreshPageViewDialog", true, infoForm);
        }else{
     ...
     *
     */
    @RequestMapping(params = {"pageId=pageFundsManagement","methodToCall=exportTaFundingPDF"})
    public ModelAndView exportTaFundingPDF(@ModelAttribute("KualiForm") UifFormBase form){
        AdminInfoForm infoForm = (AdminInfoForm) form;
        super.baseStart(infoForm);

        if (infoForm.getTaFunding() == null || infoForm.getTaFunding().size() == 1) { //size=1是因为会设置至少一个空object让表格不会消失
            if(infoForm.getTaFunding().get(0)== null) {
                infoForm.setErrMsg("列表为空！");
                return this.showDialog("refreshPageViewDialog", true, infoForm);
            }
        }
        ...
        String filePath=ipdfService.prepareTaFundingToPDF(taFundingViewObjectList);
        if (filePath.equals("exception")){
            infoForm.setErrMsg("系统导出PDF文件错误！");
            return this.showDialog("refreshPageViewDialog", true, infoForm);
        }else{
    ...
    public ModelAndView SaveClassFunding(@ModelAttribute("KualiForm") UifFormBase form   ) {
        ...
        if(code == 10){
            ...
            if(adminService.countClassFunding(draftClassFunding, totalAssignedClass) == 2) {
                ...
                String ClassFundingMessage="保存修改";
                for (ClassFundingViewObject CfViewObject :draftClassFunding)
                {
                    ClassFundingMessage+="学院是"+CfViewObject.getDepartment()+"课程是"+CfViewObject.getCourseName()+"的批准经费为"+CfViewObject.getAssignedFunding()+"、";
                }
                logger.info(ClassFundingMessage+"的课程经费");
                infoForm.setClassFundingStatistics(
                        taConverter.countClassFunding(
                                infoForm.getClassFundings(),
                                adminService.getClassTotalAssignedFunding(),
    ...
    public ModelAndView SaveTaFunding(@ModelAttribute("KualiForm") UifFormBase form   ) {
        ...
        if(code == 10){
            ...
            String TaFundingMessage="保存修改";
            for (TaFundingViewObject TfViewObject :taFundingViewObjects)
            {
                TaFundingMessage+="学院是"+TfViewObject.getDepartmentName()+"课程是"+TfViewObject.getCourseName()+"的助教"+TfViewObject.getTaName()+"的分配经费为"+TfViewObject.getAssignedFunding()+"、";
            }
            logger.info(TaFundingMessage+"的助教经费");
            return this.getModelAndView(infoForm, "pageFundsManagement");
        }
    }
    ...
    public ModelAndView SaveDeptFunding(@ModelAttribute("KualiForm") UifFormBase form   ) {
        ...
        if(code == 10){
            ...
            if(adminService.countDeptFunding(draftDepartmentFundingViewObject) == 3) {
                ...
                String DepartmentFundingMessage="保存修改";
                for (DepartmentFundingViewObject DfViewObject:draftDepartmentFundingViewObject)
                {
                    DepartmentFundingMessage+="批次是"+DfViewObject.getSessionName()+"学院是"+DfViewObject.getDepartment()+"的预算经费为"+DfViewObject.getPlanFunding()+"和批准经费为"+DfViewObject.getActualFunding()+"、";
                }
                logger.info(DepartmentFundingMessage+"的学院经费");
    ...
    public ModelAndView SaveSessionFunding(@ModelAttribute("KualiForm") UifFormBase form) {
        ...
        if(code == 10){
            ...
            String SessionFundingMessage="保存修改";
            for (SessionFundingViewObject SfViewObject:sessionFundingViewObjects)
            {
                SessionFundingMessage+="批次是"+SfViewObject.getSessionName()+"的预算经费为"+SfViewObject.getPlanFunding()+"和批准经费为"+SfViewObject.getActualFunding();
            }
            adminService.saveSessionFunding(sessionFundingViewObjects);
            logger.info(SessionFundingMessage+"的批次经费");
            infoForm.setSessionFundingStatistics(      //已设置经费/总经费
    ...
    public ModelAndView updateCourseCategory(@ModelAttribute("KualiForm") UifFormBase form,
        ...
        logger.info("修改课程类别为"+adminInfoForm.getClassification().getName());
    ...
    public ModelAndView saveTaCategory(@ModelAttribute("KualiForm") UifFormBase form) {
        ...
        if (adminInfoForm.getTaIndex() != null) {
            if (adminService.changeTaCategoryByEntiy(adminInfoForm.getOldTaCategory())) {
                ...
                logger.info("编辑助教类别为"+adminInfoForm.getOldTaCategory().getName()+"、时薪为"+adminInfoForm.getOldTaCategory().getHourlyWage());
            }
            else {
                // TODO: 2016/10/29 弹出错误提示，具体错误信息待补充
                logger.error("编辑助教类别为"+adminInfoForm.getOldTaCategory().getName()+"、时薪为"+adminInfoForm.getOldTaCategory().getHourlyWage()+"失败");
    ...
    public ModelAndView saveTerm(@ModelAttribute("KualiForm") UifFormBase form) throws ParseException {
        ...
        if (adminInfoForm.getTermIndex() != null) {
            ...
            logger.info("编辑批次名称为"+adminInfoForm.getCurrentTerm().getTermName()+"、开始时间为"+adminInfoForm.getCurrentTerm().getStartDate()+"、结束时间为"+adminInfoForm.getCurrentTerm().getEndDate()+"。");
        } else {
            // add
            //同时进行批次和学院经费初始化
            if(adminInfoForm.getCurrentTerm().getStartDate()==null||adminInfoForm.getCurrentTerm().getEndDate()==null){
